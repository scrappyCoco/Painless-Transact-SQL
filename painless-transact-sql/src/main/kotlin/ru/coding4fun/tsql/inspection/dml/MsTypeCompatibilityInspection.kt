...
class MsTypeCompatibilityInspection : SqlInspectionBase() {
    ...
    private class TypeCompareVisitor(manager: InspectionManager,
        ...
        override fun visitSqlBinaryExpression(binaryExpression: SqlBinaryExpression?) {
            visitSqlBinaryExpressionImpl(binaryExpression)
            super.visitSqlBinaryExpression(binaryExpression)
        }

        private fun visitSqlBinaryExpressionImpl(binaryExpression: SqlBinaryExpression?) {
            if (binaryExpression == null || binaryExpression.rOperand == null) return
            if (notAvailableOperators.contains(binaryExpression.opSignElement.elementType)) return
            val lTypeName: String? = getTypeName(binaryExpression.lOperand)
            val rTypeName: String? = getTypeName(binaryExpression.rOperand!!)
            if (lTypeName == null || rTypeName == null) return
        ...
        override fun visitSqlInsertStatement(insertStatement: SqlInsertStatement?) {
            visitSqlInsertStatementImpl(insertStatement)
            super.visitSqlInsertStatement(insertStatement)
        }

        fun visitSqlInsertStatementImpl(insertStatement: SqlInsertStatement?) {
            ...
            val targetColumnList = insertDmlInstruction.columnsList?.columnsReferenceList?.referenceList
                    ?: return

            val selectList = insertDmlInstruction.queryExpression?.selectClause?.expressions?.toList()
            if (selectList != null) {
                checkColumnList(targetColumnList, selectList)
                return
            }

            val valuesList = insertDmlInstruction.valuesExpression?.expressions?.toList() ?: return
            for (valueList in valuesList) {
                val expressionList = (valueList as? SqlParenthesizedExpression)?.expressionList
                checkColumnList(targetColumnList, expressionList)
            }
        }

        private fun checkColumnList(
                targetColumnList: MutableList<SqlReferenceExpression>,
                valuesList: List<SqlExpression>?) {
            if (valuesList == null || targetColumnList.size != valuesList.size) return
        ...
        private fun match(lType: String?, rType: String?): MatchResult {
            ...
            val message: String? = when (convertType) {
                ConvertType.Explicit -> MsInspectionMessages.message("type.compatibility.explicit", lType, rType)
                ConvertType.Implicit -> MsInspectionMessages.message("type.compatibility.implicit", lType, rType)
                ConvertType.Restrict -> MsInspectionMessages.message("type.compatibility.restrict", lType, rType)
                else -> null
            }
